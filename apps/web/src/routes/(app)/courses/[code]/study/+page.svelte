  ...
  {#if mode === "select"}
    ...
    <div class="space-y-3">
      <div class="text-center py-4">
        ...
        <h2 class="text-xl font-black text-foreground">{courseCode} Study Session</h2>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="rounded-2xl bg-muted/40 border border-border p-3 text-center">
          <p class="text-2xl font-black text-foreground">{stats.flashcardCount}</p>
      ...
      <div class="space-y-3 pt-2">
        ...
        >
          <div class="size-12 rounded-2xl bg-brand-500 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
            <Brain class="size-6 text-white" />
          </div>
          <div>
            <p class="text-sm font-bold text-foreground">Flashcard Deck</p>
            <p class="text-xs text-muted-foreground">Spaced repetition — flip cards to reinforce memory</p>
          </div>
        </button>
        ...
        <button
          ...
          onclick={() => (mode = "quiz")}
        >
          <div class="size-12 rounded-2xl bg-emerald-500 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
            <BookOpen class="size-6 text-white" />
          </div>
          <div>
            <p class="text-sm font-bold text-foreground">Quiz Session</p>
            <p class="text-xs text-muted-foreground">Timed MCQ — test your knowledge with a score</p>
          </div>
        </button>
        ...
        >
          <div class="size-12 rounded-2xl bg-amber-500 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
            <Zap class="size-6 text-white" />
          </div>
          <div>
            <p class="text-sm font-bold text-foreground">Mock Exam</p>
            <p class="text-xs text-muted-foreground">Full-length timed exam (MCQ, Theory, Objective)</p>
          </div>
        </a>
      </div>
    </div>
  ...
  {:else if mode === "flashcards"}
    <div class="space-y-4">
      <div class="flex items-center gap-2">
        <button onclick={() => (mode = "select")} class="text-muted-foreground hover:text-foreground">
          <RotateCcw class="size-4" />
        </button>
        <h2 class="text-sm font-bold text-foreground">Flashcard Deck — {courseCode}</h2>
      </div>
        ...
        {:then cards}
          ...
          {:else}
            <div class="flex flex-col items-center gap-3 py-16 text-center">
              ...
              <p class="text-sm text-muted-foreground">No flashcards yet for {courseCode}.</p>
  ...
  {:else if mode === "quiz"}
    <div class="space-y-4">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-sm font-bold text-foreground">Quiz Session — {courseCode}</h2>
      </div>
        ...
        {:then qs}
          ...
          {:else}
            <div class="flex flex-col items-center gap-3 py-16 text-center">
              ...
              <p class="text-sm text-muted-foreground">No quizzes yet for {courseCode}.</p>
              <Button size="sm" onclick={() => (mode = "select")}>Back</Button>
            </div>
          {/if}
        {/await}
