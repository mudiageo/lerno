<script lang="ts">
  ...
  const result = await getFeed({ cursor: undefined, courseCode, postType: "short" });
  ...
  function next() { if (currentIndex < shorts.length - 1) currentIndex++; }
...
<div class="relative h-[calc(100vh-120px)] overflow-hidden bg-black flex flex-col">
  {#if shorts.length === 0}
    <div class="flex flex-col items-center justify-center h-full gap-4 text-center px-6">
      ...
      <div>
        ...
        <p class="text-white/50 text-sm mt-1">AI video flashcards for {courseCode} will appear here.</p>
      </div>
    </div>
  {:else}
    <!-- Short card -->
        ...
        >
          ...
          {#if current?.content?.front}
            <div class="max-w-sm w-full space-y-6">
              <div class="size-16 rounded-2xl bg-white/10 flex items-center justify-center mx-auto">
                <Brain class="size-8 text-white" />
              </div>
              <div>
                <p class="text-white/60 text-xs font-medium uppercase tracking-widest mb-3">
                  {current.topicTags?.[0] ?? courseCode}
                </p>
                <p class="text-white text-2xl font-black leading-tight">{current.content.front}</p>
              </div>
              <div class="p-4 rounded-2xl bg-white/10 border border-white/20 text-left">
                <p class="text-white/50 text-xs font-bold uppercase mb-1">Answer</p>
                <p class="text-white/90 text-sm leading-relaxed">{current.content.back}</p>
              </div>
            </div>
          {:else if current?.content?.body}
            <p class="text-white text-xl font-bold leading-snug max-w-sm">{current.content.body}</p>
          {/if}
        </div>
      {/key}
    ...
    <div class="absolute right-4 bottom-24 flex flex-col items-center gap-5">
      <button class="flex flex-col items-center gap-1" onclick={() => (muted = !muted)}>
      ...
      <button class="flex flex-col items-center gap-1">
        ...
        <span class="text-white text-[11px] font-medium">{current?.likeCount ?? 0}</span>
      </button>
      <button class="flex flex-col items-center gap-1">
        <div class="size-10 rounded-full bg-white/20 flex items-center justify-center">
          <MessageCircle class="size-5 text-white" />
        </div>
        <span class="text-white text-[11px] font-medium">{current?.replyCount ?? 0}</span>
      </button>
      <button class="flex flex-col items-center gap-1">
        <div class="size-10 rounded-full bg-white/20 flex items-center justify-center">
    ...
    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-1.5">
      {#each shorts as _, i}
        <div class="rounded-full transition-all {i === currentIndex ? 'w-4 h-1.5 bg-white' : 'size-1.5 bg-white/40'}"></div>
      {/each}
    </div>

    <!-- Counter -->
    <div class="absolute top-4 right-4 bg-black/40 px-2 py-1 rounded-full text-white text-xs font-mono">
