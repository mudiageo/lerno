<script lang="ts">
  ...
  async function handleSignUp(e: Event) {
    ...
    await signUp.email({
      email, password, name,
      username: email.split('@')[0], // we'll let them customize in onboarding
      fetchOptions: {
...
<div class="max-w-md mx-auto mt-20 p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
  ...
  <form onsubmit={handleSignUp} class="space-y-4">
    <div>
      <label class="block text-sm font-medium mb-1 text-zinc-700 dark:text-zinc-300">Full Name</label>
      <input type="text" bind:value={name} required class="w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-transparent" />
    </div>
    ...
    <div>
      <label class="block text-sm font-medium mb-1 text-zinc-700 dark:text-zinc-300">Password</label>
  ...
  <p class="mt-6 text-center text-sm text-zinc-600 dark:text-zinc-400">
    Already have an account? <a href="/sign-in" class="font-medium text-zinc-900 dark:text-white hover:underline">Sign in</a>
  </p>
</div>
