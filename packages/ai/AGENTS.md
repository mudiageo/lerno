...
### Core Orchestrator
- [BananaOrchestrator](./src/orchestrator/banana.ts): Coordinates LLM planning, asset generation, and assembly.
...
### Provider Interfaces
1. [LlmProvider](./src/interfaces/llm.ts): Text & JSON generation (Native Gemini 3.1).
2. [ImageProvider](./src/interfaces/image.ts): Visual generation (Nano Banana).
3. [TtsProvider](./src/interfaces/tts.ts): Narration (Gemini TTS).
4. [AudioProvider](./src/interfaces/audio.ts): Background Music (Lyria).
5. [VideoGenerator](./src/interfaces/video.ts): Final assembly (FFmpeg).

### Environment Configuration
Switch between `google` and `simulated` using environment variables handled by the [ProviderFactory](./src/factory.ts):
- `AI_LLM_PROVIDER`
- `AI_IMAGE_PROVIDER`
- `AI_TTS_PROVIDER`
- `AI_AUDIO_PROVIDER`
...
- **Valibot ONLY**: Do NOT use `zod`. This package uses [Valibot](./package.json#L35) for schema validation.
