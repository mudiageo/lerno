<script lang="ts">
  ...
  import {
    ...
    joinCommunity,
    leaveCommunity,
  } from "../../../communities/communities.remote";
...
<div class="px-4 py-5 space-y-5">
    ...
    <div class="flex flex-col items-center gap-5 py-16 text-center">
      ...
      <div>
        ...
        <p class="text-sm text-muted-foreground mt-1 max-w-xs">
          Create a community for {courseCode} students to share notes, ask questions,
          and study together.
        </p>
      </div>
      <a href="/communities">
  ...
  {:else}
    ...
    <div class="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      {:else}
        <div
          class="w-full h-20 bg-gradient-to-br from-brand-400 to-brand-600"
        ></div>
      {/if}
      <div class="px-4 pb-4 -mt-5 flex items-end justify-between gap-3">
        <div
          class="size-14 rounded-2xl bg-white dark:bg-background border-2 border-background shadow-lg flex items-center justify-center text-2xl"
        >
          👥
        </div>
        <Button
      ...
      <div class="px-4 pb-3">
        <a
          ...
          class="text-base font-bold text-foreground hover:underline"
        >
          {community.name}
        </a>
        <p class="text-xs text-muted-foreground mt-0.5">
          {community.memberCount} members · {courseCode}
        </p>
      </div>
    </div>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        <a
          href="/communities/{community.slug}"
          class="text-xs text-brand-500 hover:underline">See all</a
        >
      </div>

      {#if communityPosts.length === 0}
        <div
          class="flex flex-col items-center gap-2 py-10 text-center rounded-xl border border-dashed border-border"
        >
      ...
      {:else}
        <div class="space-y-2">
          {#each communityPosts.slice(0, 5) as post (post.id)}
            <div
              class="p-3.5 rounded-xl border border-border/70 bg-card hover:bg-accent/30 transition-colors"
            >
              <div class="flex items-center gap-2 mb-2">
                <div
                  class="size-7 rounded-full bg-brand-100 dark:bg-brand-950/40 flex items-center justify-center text-xs font-bold text-brand-600 overflow-hidden"
                >
              ...
              >
                <span class="flex items-center gap-1"
                  ><Heart class="size-3" />{post.likeCount ?? 0}</span
                >
                <span class="flex items-center gap-1"
                  ><MessageCircle class="size-3" />{post.replyCount ?? 0}</span
                >
                <Badge variant="outline" class="text-[10px] h-4 px-1 ml-auto"
                  >{post.postType}</Badge
                >
              </div>
            </div>
